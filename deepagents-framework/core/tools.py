...
class EditFileSchema(BaseModel):
    ...
    old_string: Optional[str] = Field(default=None, description="The string to be replaced or the regex pattern. Required for 'replace' and 'regex_replace' modes.")
    new_string: Optional[str] = Field(default=None, description="The new string to insert or replace with.")
    insert_line: Optional[int] = Field(default=None, description="The line number at which to insert the new_string. Used in 'insert' mode.")
    insert_after: Optional[str] = Field(default=None, description="A substring in the file. The new_string will be inserted on the line immediately after the first occurrence of this substring. Used in 'insert' mode.")
...
def get_data_dictionary(
    ...
    if _cached_data_dictionary is None:
        ...
        for name, path in file_paths.items():
            try:
                dataframes[name] = pd.read_csv(path, parse_dates=True)
